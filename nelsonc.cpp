...
void nelsonc::schritt()
{
    ...
    {
       ruckstop=pos[1];
       pos[0]=pos[0]+(pos[0]*((ti-a)/(a*a+ti*ti))-((sinh(2.0*a*s*pos[0]/(a*a+ti*ti))+sin(2.0*ti*s*pos[0]/(a*a+ti*ti)))/(cosh(2.0*a*s*pos[0]/(a*a+ti*ti))+cos(2.0*ti*s*pos[0]/(a*a+ti*ti))))*s*((ti-a)/(a*a+ti*ti)))*dt+gauss(0.0,1.0)*sqrt(dt);
       pos[1]=pos[1]+(((ti-a)/(a*a+(ti)*(ti)))*pos[1]+8.0*((a*(a+ti))/(a*a+(ti)*(ti))))*dt+gauss(0.0,1.0)*sqrt(dt);
       ti=ti+0.3;
